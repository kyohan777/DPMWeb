...
var modDpmDailyPro = (function(){    
	...
	function init() {
		...
		$("#jqGrid").jqGrid({
	        ...
	        //jqGrid 양식선언부        
	        colModel: [
	            { label: '연번', 	  name: 'prcDt', width: 100, align: 'center', formatter:currencyFmatterDate},
	            { label: 'EI_ID', name: 'bizBsnNm', width: 60, align: 'center' },
	            { label: 'IMG_FILE_NAME', name: 'verifySts', width: 50, align: 'center'},
		...
		modAjax.request("/dpm/insFpMaskHis.do", objParam, {
			...
			success : function(cnt) {
				...
				if($("#selCategory").val() != "5") {
					...
					modDpmDailyPro.selListPage(curPage,pageSize);
...
$("#selThumbnailSize").on("change", function() {
	modDpmDailyPro.setThumbnailSize($("#selThumbnailSize").val());
});
